...
def testReport(request):
    ...
    if request.FILES.get('s1_file'):
        ...
        if excel.find({"小区中文名": s1_name}).count() == 0:  # 查询小区是否存在
    ...
    if request.FILES.get('s2_file'):
        ...
        if excel.find({"小区中文名": s2_name}).count() == 0:  # 查询小区是否存在
    ...
    if request.FILES.get('s3_file'):
        ...
        if excel.find({"小区中文名": s3_name}).count() == 0:  # 查询小区是否存在
...
def getMessageFromMongodb(village_name):
    for i in excel.find({"小区中文名": village_name}):
        Lontitude = i[u'经度']
        Latitude = i[u'纬度']
        CGI = i['CGI']
